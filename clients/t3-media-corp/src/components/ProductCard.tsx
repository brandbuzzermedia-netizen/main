  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute left-4 top-4 bg-ink/80 px-3 py-1.5 font-mono text-[0.625rem] font-medium uppercase tracking-eyebrow text-bronze-light backdrop-blur-sm">
          {product.category}
        </span>

        <span className="absolute bottom-4 right-4 flex h-10 w-10 translate-y-2 items-center justify-center bg-bronze-light text-ink opacity-0 transition-all duration-500 ease-editorial group-hover:translate-y-0 group-hover:opacity-100">
      ...
      <div className="flex flex-1 flex-col pt-6">
        <div className="flex items-baseline justify-between gap-4">
          <h3 className="display-3 relative inline-block text-paper">
            {product.name}
            <span className="absolute -bottom-1 left-0 h-px w-full origin-left scale-x-0 bg-bronze-light transition-transform duration-500 ease-editorial group-hover:scale-x-100" />
          </h3>
          {index !== undefined && (
            <span className="shrink-0 font-mono text-[0.75rem] leading-none text-stone">
              {String(index).padStart(2, '0')}
            </span>
          )}
        </div>
        <p className="mt-3.5 max-w-sm text-[0.9375rem] leading-relaxed text-mist">
