...
`platform/` holds **Social OS** — a multi-tenant AI social media operating
system for running client social media at agency scale: trend research,
inspiration, engagement opportunities, AI content and comment drafting, an
approval centre, and publishing through official platform APIs.

It is a separate application from this marketing site and shares nothing but the
brand palette.

```bash
cd platform && npm run migrate && npm run seed && npm start
```
